...
# Updating data.path for the 2023 data
data.path <- "input_data/morton_dendrometer_2023/"
...
# listing out all fo the files in the directory
group.files <- list.files(data.path)
group.files <- group.files[!group.files %in% "morton_dendrometer_cleaned.csv"] # we make the 'morton_dendrometer_cleaned' file later and this is so that we don't read it in if we need to rerun the extraction.
...
# saving dendrometer band data.
saveRDS(mort.dendro.dat2, file = "processed_data/morton_dendrometer_cleaned_2023.rds")
write.csv(mort.dendro.dat2, file.path(data.path, "morton_dendrometer_cleaned_2023.csv"), row.names=F)
